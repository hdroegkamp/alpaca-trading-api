...
def main():
    parser = argparse.ArgumentParser(description='Run vectorized backtest on historical data')
    parser.add_argument('symbol', help='Trading symbol to backtest')
    parser.add_argument('--strategy', default='MovingAverageCrossover',
    ...
    parser.add_argument('--fast-window', type=int, help='Fast MA window (for MA crossover)')
    parser.add_argument('--slow-window', type=int, help='Slow MA window (for MA crossover)')
    parser.add_argument('--window', type=int, help='Window for mean reversion')
    parser.add_argument('--num-std', type=float, help='Number of std devs for Bollinger Bands')
    ...
    try:
        ...
        if data is None:
            logger.error(f"No data found for {args.symbol}. Run download_data.py first.")
            sys.exit(1)
